...
説明文の中には、必ず対象となる【キーワード】（例：【registry_write】）を明記し、
それがどのように実際のマルウェアの挙動に結びついているかを強調してください。

### タスク2: Behavior Synthesis（挙動要約）
与えられたエビデンスを統合し、このラベルが示すマルウェアの挙動を
簡潔かつ論理的に要約してください。
要約の中にも、必ず重要な【キーワード】を織り込んで説明してください。
...
2. SHAP値が高い特徴量について、なぜそれがこの検体の分類に重要だったのかを説明してください。
   その際、説明文の中に必ず重要と判定された【キーワード】（例：【file_created】）を明記し、
   それらがどのように一連の攻撃シナリオを構成しているかを説明してください。
3. この検体の全体的な攻撃シナリオを要約してください。要約の中にも重要な【キーワード】を織り込んでください。
